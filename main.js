(() => {
    ...
    const TRANSLATIONS = {
        en: {
            ...
            'case.dsskills.subtitle': 'Turning a solid but unused design system into a rapid-prototyping engine. Two AI skills let a PM build a client-ready Figma prototype in a single morning, from a plain-language brief.',
            ...
            'case.dsskills.metric2.label': 'operator + builder, design-system aware',
            'case.dsskills.metric3.value': '11 screens · 4 modals',
            'case.dsskills.metric3.label': 'generated from a brief, no manual design step',
            'case.dsskills.problem':    '<p>It is a tension every B2B designer knows. The product moves fast, clients expect quick answers, and the prototype always lands too late, or too far from the real product.</p><p>At Oplit we build software for industrial production planning. The cycle looked like this: a PM frames a need, we design in Figma, we try to stay consistent with the design system, we ship a prototype that looks like the product but is not the product, engineering re-decides things design could have settled, and every handoff creates friction. The skills were there; the chain between them was the problem.</p><p>So I asked a simple question: can we design prototypes so faithful to the design system that the design-to-dev gap nearly disappears? And fast enough that a PM with no design training could build one alone?</p>',
            'case.dsskills.approach':   '<p>Before building anything, I needed the real state of the DS. Not by intuition. I ran a <a href="#project-ds-audit" data-page="project-ds-audit">full audit of Opal</a>: internal health was far better than we thought (color almost fully tokenised, near-complete documentation, 13 of 13 components accessible), but product adoption was very low, around 8% of instances coming from the DS. The system existed and was solid, yet almost no one used it.</p><p>I then ran the <a href="#project-ds-execution" data-page="project-ds-execution">corrective actions</a> with Claude Code and the Figma MCP to operate the DS directly: 636 spacings and 66 radii bound to their tokens, full documentation across 53 public components, the Figma to Notion mapping restored. Nothing propagated without a guardrail. My rule: no change reaches the DS without a validated manifest and my explicit approval.</p><p class="case-caption">Every axis is measured live from the Figma plugin, not estimated. The gap between design, dev and product is the case for the project: the DS is solid on the design side and almost invisible on the product side.</p>',
            'case.dsskills.solution':   '<p>Engineering was loaded and Storybook was waiting on resources. Instead of waiting, I went around it. The idea: if engineering cannot come to the design system, the design system has to go to the teams.</p><p>I built two skills, specialised AI agents. The first, the <strong>operator</strong>, holds the knowledge: where components live, their variants, how they assemble, the rules that govern their use, and it updates itself as the DS evolves. The second, the <strong>builder</strong>, consumes that knowledge to construct screens. You give it a plain-language brief, for example show the impact of format-change matrices on scheduling, and it instantiates the right components, in the right variants, with the right tokens, directly in Figma.</p><p>The method mattered as much as the result. Every fact about the DS was verified live before being encoded into the skill. Every version was tested by a blank agent with no memory, to simulate someone starting from scratch: if it recovered the right knowledge from the live sources, the skill was valid, otherwise we iterated. The first build produced structurally correct but uniform interfaces, with no granularity. That told me the skill needed more context on variants and contextual usage rules. The second build instantiated the right variants based on the real state of each work order. Not perfect, but credible for a client presentation.</p>',
            'case.dsskills.outcome':    '<p>The best validation did not come from my own tests. My PM took the builder. No design training, no knowledge of the Figma components, without going through me. In one morning he built a prototype of the format-change matrices feature, a case that lets schedulers see the impact of item sequences on their OEE in real time, get alerted on forbidden changes, and replace their Excel sheets with an integrated interface. A few hours later he was presenting it to a client.</p><p>Here is what he wrote to the team right after: <em>"A huge thank you for building this prototype. The numbers and calculations are wrong, but not stupid."</em></p><p>Wrong, but not stupid. That is the whole point. A useful prototype only has to be credible. The credibility came from real components, coherent interactions and an exact visual language, so the wrong numbers did not matter. The DS had done its job, invisibly.</p>',
            'case.dsskills.reflection': '<p>Both skills are agnostic. I did not build them for Oplit alone. I separated the functional core from the Oplit knowledge, so the operator and the builder leak nothing about our components or our business rules. They point to configurable sources: a Figma key, a Notion base, a git repository.</p><p>The biggest shift is the design-to-dev handoff. Because the prototype is built from the real design system, engineering receives screens made of the actual components and tokens, with nothing to reinterpret. Fewer re-decisions, less back-and-forth, a shorter path from design to shipped code.</p><p>Any design team with a documented DS can deploy them on its own system. The chain becomes: plain-language brief, real components, faithful prototype, a handoff that holds. The scope goes past Oplit: a reproducible way of working for any design team that wants to take AI seriously without giving up the craft.</p>',
            'case.dsskills.questions':  '<p>This project solves a concrete problem, but it opens wider ones that I would rather face than dodge.</p><ul class="case-list"><li>What does craft mean in a job where you can reach near-identical screens in a few minutes?</li><li>What posture should a designer take with this kind of tool?</li><li>Which workflow becomes the most relevant with this new ally, one that is still painful at times?</li><li>When is it worth moving forward with AI, and when is it better to work without it?</li><li>What does this ask me to change in the way I approach design?</li><li>How do we design the AI experience itself, including onboarding someone into using an AI skill?</li><li>And the most uncomfortable one: if I hold this skill through my tool, do I genuinely acquire it myself?</li></ul><p>I do not have all the answers. I would rather work these questions from the inside than watch them from a distance.</p>',
            'case.dsskills.learning':   '<p>Working this way forced me to be far more explicit about what I know. Encoding rules into a skill means stating them clearly, not keeping them as intuition. It made me settle ambiguities I had let sit for months. "Is this component used in this context?" The answer cannot be "it depends" when you are configuring an agent.</p><p>It also confirmed where the value of a senior designer sits here: knowing what to hand the tools and what to keep. I treated the guardrail, where nothing propagates without a validated manifest, as a design decision. The system would have been faster without it, and less reliable and riskier for the integrity of the DS. Speed is a tool, rigor is a posture. This project only has value because the two coexist.</p><p class="case-caption">Stack: Claude Code (operating the DS and building the skills), Figma MCP (live Figma connector from the agent), Figma (Opal file, components and foundations), Notion (knowledge base and documentation), a human validation gate on every propagation.</p>',
            'case.dsskills.fig.dashboard': 'The diagnosis on one screen: Design 97, Dev 25, Product 10. The imbalance is the case for the project.',
            'case.dsskills.fig.design':    'Design axis, composite score 97/100, measured live from the plugin.',
            'case.dsskills.fig.dev':       'Dev axis: tokens exported and ready, Storybook waiting on resources.',
            'case.dsskills.fig.product':   'Product axis: the DS still thin in the app, first AI-assisted build under way.',
            'case.dsskills.fig.versions':  'The same screen in three states: the real product, build v1, build v2 after iterating on the skill.',
            'case.dsskills.fig.flow1':     'A full 4-step flow (interactions, sticky bar, a modal with real Form components) generated from a brief.',
        ...
        fr: {
            ...
            'case.dsskills.subtitle': 'Transformer un design system solide mais peu utilisé en moteur de prototypage rapide. Deux skills IA permettent à un PM de construire un prototype présentable à un client en une matinée, à partir d\'un brief en langage naturel.',
            'case.dsskills.metric1.value': '1 matinée',
            'case.dsskills.metric1.label': 'd\'un brief à un prototype montrable à un client',
            'case.dsskills.metric2.value': '2 skills',
            'case.dsskills.metric2.label': 'operator + builder, qui connaissent le design system',
            ...
            'case.dsskills.problem':    '<p>Il y a une tension que tout designer B2B connaît. Le produit avance vite, les clients attendent des réponses rapides, et le prototype arrive toujours trop tard, ou trop loin du produit réel.</p><p>Chez Oplit, on développe un logiciel de pilotage de production industrielle. Le cycle ressemblait à ça : un PM exprime un besoin, on conçoit dans Figma, on essaie de rester cohérent avec le design system, on livre un prototype qui ressemble au produit mais qui n\'en est pas un, le dev refait des décisions que le design aurait pu fixer, et chaque handoff crée des frictions. Les compétences étaient là ; le problème, c\'était la chaîne entre elles.</p><p>D\'où une question simple : peut-on concevoir des prototypes si fidèles au design system que l\'écart entre design et dev disparaît presque ? Et assez vite pour qu\'un PM sans formation design puisse en créer un seul, de son côté ?</p>',
            'case.dsskills.approach':   '<p>Avant de construire quoi que ce soit, il fallait connaître l\'état réel du DS. Pas à l\'intuition. J\'ai conduit un <a href="#project-ds-audit" data-page="project-ds-audit">audit complet d\'Opal</a> : une santé interne bien meilleure qu\'on ne le pensait (couleur presque entièrement tokenisée, documentation quasi complète, 13 composants accessibles sur 13), mais une adoption produit très basse, autour de 8% d\'instances issues du DS. Le système existait, il était solide, et pourtant presque personne ne s\'en servait.</p><p>J\'ai ensuite mené les <a href="#project-ds-execution" data-page="project-ds-execution">actions correctives</a> avec Claude Code et le MCP Figma pour opérer le DS directement : 636 espacements et 66 rayons reliés à leurs tokens, documentation complète sur 53 composants publics, bijection Figma vers Notion rétablie. Rien ne se propageait sans garde-fou. Ma règle : aucune modification n\'atteint le DS sans manifeste validé et accord explicite de ma part.</p><p class="case-caption">Chaque axe est mesuré en live depuis le plugin Figma, pas estimé. L\'écart entre design, dev et produit justifie le projet : le DS est solide côté design et presque invisible côté produit.</p>',
            'case.dsskills.solution':   '<p>Le dev était chargé et Storybook attendait des ressources. Plutôt que d\'attendre, j\'ai contourné. L\'idée : si le dev ne peut pas venir au design system, le design system doit aller vers les équipes.</p><p>J\'ai créé deux skills, des agents IA spécialisés. Le premier, l\'<strong>operator</strong>, porte la connaissance : où vivent les composants, leurs variantes, comment ils s\'assemblent, les règles qui régissent leur usage, et il se met à jour quand le DS évolue. Le second, le <strong>builder</strong>, consomme cette connaissance pour construire des écrans. On lui donne un brief en langage naturel, par exemple montrer l\'impact des matrices de changement de format sur l\'ordonnancement, et il instancie les bons composants, dans les bonnes variantes, avec les bons tokens, directement dans Figma.</p><p>La démarche comptait autant que le résultat. Chaque fait sur le DS a été vérifié en live avant d\'être encodé dans le skill. Chaque version a été testée par un agent vierge, sans mémoire, pour simuler quelqu\'un qui part de zéro : s\'il retrouvait la bonne connaissance depuis les sources live, le skill était valide, sinon on itérait. Le premier build générait des interfaces structurellement justes mais uniformes, sans granularité. Ça m\'a montré que le skill avait besoin de plus de contexte sur les variantes et leurs règles d\'usage. Le second build instanciait les bonnes variantes selon l\'état réel de chaque ordre de fabrication. Pas parfait, mais crédible pour une présentation client.</p>',
            'case.dsskills.outcome':    '<p>La meilleure validation n\'est pas venue de mes propres tests. Mon PM a pris le builder. Sans formation design, sans connaître les composants Figma, sans passer par moi. En une matinée, il a construit un prototype de la feature matrices de changement de format, un cas qui permet aux ordonnanceurs de visualiser l\'impact des séquences d\'articles sur leur TRS en temps réel, d\'être alertés sur les changements interdits, et de remplacer leurs tableurs Excel par une interface intégrée. Quelques heures plus tard, il le présentait à un client.</p><p>Voici ce qu\'il a écrit à l\'équipe dans la foulée : <em>« Un gigantesque merci pour la construction de ce prototype. Les chiffres et calculs sont faux, mais pas idiots. »</em></p><p>Faux, mais pas idiots. C\'est tout l\'enjeu. Un prototype utile n\'a qu\'à être crédible. La crédibilité venait de composants réels, d\'interactions cohérentes et d\'un langage visuel exact, donc les chiffres faux ne gênaient pas. Le DS avait fait son travail, de façon invisible.</p>',
            'case.dsskills.reflection': '<p>Les deux skills sont agnostiques. Je ne les ai pas construits pour Oplit uniquement. J\'ai séparé le cœur fonctionnel de la connaissance Oplit, pour que l\'operator et le builder ne laissent rien fuir de nos composants ou de nos règles métier. Ils pointent vers des sources configurables : une clé Figma, une base Notion, un dépôt git.</p><p>Le vrai changement, c\'est le handoff entre design et dev. Comme le prototype est construit à partir du vrai design system, le dev reçoit des écrans faits des composants et des tokens réels, sans rien à réinterpréter. Moins de décisions refaites, moins d\'allers-retours, un chemin plus court entre le design et le code livré.</p><p>N\'importe quelle équipe design avec un DS documenté peut les déployer sur son propre système. La chaîne devient : brief en langage naturel, composants réels, prototype fidèle, un handoff qui tient. La portée dépasse Oplit : un modèle de travail reproductible pour toute équipe design qui veut prendre l\'IA au sérieux sans renoncer au craft.</p>',
            'case.dsskills.questions':  '<p>Ce projet règle un problème concret, mais il en ouvre d\'autres, plus larges, que je préfère affronter plutôt qu\'esquiver.</p><ul class="case-list"><li>Que veut dire le craft dans un métier où l\'on peut atteindre des écrans quasi identiques en quelques minutes ?</li><li>Quelle posture le designer doit-il adopter face à ce type d\'outil ?</li><li>Quel workflow devient le plus pertinent avec ce nouvel allié, qui reste pénible par moments ?</li><li>Quand a-t-on intérêt à avancer avec l\'IA, et quand vaut-il mieux faire sans ?</li><li>Quelles adaptations ça me demande dans ma façon d\'aborder le design ?</li><li>Comment penser l\'AI experience elle-même, et notamment l\'onboarding à l\'usage d\'une compétence IA ?</li><li>Et la plus inconfortable : si je détiens cette compétence à travers mon outil, est-ce que je l\'acquiers vraiment moi-même ?</li></ul><p>Je n\'ai pas toutes les réponses. Je préfère travailler ces questions de l\'intérieur plutôt que les regarder de loin.</p>',
            'case.dsskills.learning':   '<p>Travailler comme ça m\'a forcé à être beaucoup plus explicite sur ce que je sais. Encoder des règles dans un skill, c\'est les formuler clairement, pas les garder en intuition. Ça m\'a obligé à trancher des ambiguïtés que je laissais traîner depuis des mois. « Est-ce que ce composant s\'utilise dans ce contexte ? » La réponse ne peut pas être « ça dépend » quand on configure un agent.</p><p>Ça m\'a aussi confirmé où est la valeur du designer senior dans ce contexte : savoir ce qu\'on confie aux outils et ce qu\'on garde pour soi. J\'ai traité le garde-fou, où rien ne se propage sans manifeste validé, comme une décision de design. Le système aurait été plus rapide sans lui, et moins fiable et plus risqué pour l\'intégrité du DS. La vitesse est un outil, la rigueur est une posture. Ce projet n\'a de valeur que parce que les deux coexistent.</p><p class="case-caption">Stack : Claude Code (opération du DS et build des skills), MCP Figma (connecteur Figma live depuis l\'agent), Figma (fichier Opal, composants et fondations), Notion (base de connaissance et documentation), une validation humaine obligatoire sur toute propagation.</p>',
            'case.dsskills.fig.dashboard': 'Le diagnostic en un seul écran : Design 97, Dev 25, Produit 10. Le déséquilibre justifie le projet.',
            'case.dsskills.fig.design':    'Axe Design, score composite 97/100, mesuré en live depuis le plugin.',
            'case.dsskills.fig.dev':       'Axe Dev : tokens exportés et prêts, Storybook en attente de ressources.',
            'case.dsskills.fig.product':   'Axe Produit : le DS encore peu présent dans l\'app, première construction assistée par IA en cours.',
            'case.dsskills.fig.versions':  'Le même écran en trois états : le produit réel, le build v1, le build v2 après itération sur le skill.',
            'case.dsskills.fig.flow1':     'Un parcours complet en 4 étapes (interactions, sticky bar, modale avec vrais composants Form) généré depuis un brief.',
